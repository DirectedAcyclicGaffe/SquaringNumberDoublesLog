...
void Corkboard::addInternalWiresToSelection()
{
    ...
    {
        std::vector<QUuid> a = casings[i]->casingBackend()->casingState().wireIds(PlugType::IN);
    ...
    for(ulong i = 0; i < casings.size(); i++)
    {
        std::vector<QUuid> a = casings[i]->casingBackend()->casingState().wireIds(PlugType::OUT);
...
Wire *Corkboard::createWireBypassUndoStack(const QJsonObject &jsonObj)
{
    ...
    int outPlugIndex = wireBackend->getPlugIndex(PlugType::OUT);
    wire->setColor(wireBackend.get()->getCasing(PlugType::OUT)->casing().outPlugLabelColors().at(outPlugIndex));
